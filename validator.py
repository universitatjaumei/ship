...
class PomXMLValidationRule:
    ...
    def execute(self, project, module):
        if module.get_type() in ['webapp', 'servicio']:
            if not module.get_final_name():
                raise Exception(
                    "The 'finalName' attribute couldn't be found in pom.xml file")
...
class JDBCUrlRemoteCheck:
    ...
    def execute(self, project, environment):
        ...
        if app_properties.find(JDBCURL) == -1:
            raise Exception(
                "The JDBCUrl syntax in remote %s environment /etc/uji/%s/app.properties is not valid." % (environment.get_environment_id(), project.get_project_id()))
            ...
            for root, dirs, files in os.walk(rootdir):
                for file in files:
                    if self.isJavascript(file) and self.fileHasConsoleLog(os.path.join(root, file)):
                        raise Exception(
                            "The file %s has console.log commands inside." % os.path.join(root, file))
...
class ConfigFileValidationRule:
    ...
    def execute(self, project, module):
        app_properties = project.get_app_properties()

        if not os.path.isfile(app_properties) or not os.access(app_properties, os.R_OK):
            raise Exception(
                "Can't read %s file or does not exists" % app_properties)
        ...
        for needle, found in needles.items():
            if not found:
                raise Exception("The value of %s is not set properly for production deploy on %s file." % (
                    needle, app_properties))


class CompiledPackageExistsValidationRule:

    def execute(self, project, module):
        file = module.get_compiled_filename()
...
class WebXmlValidationRule:
    ...
    def execute(self, project, module):
        ...
        local("unzip -d %s/target/verify %s/target/%s.war" %
              (module.get_directory(), module.get_directory(), module.get_final_name()))

        if not os.path.isfile("%s/target/verify/WEB-INF/web.xml" % module.get_directory()):
            logging.warning(
                Colors.WARNING + "[WARN] web.xml file can not be found. Servlets 3.0 module?" + Colors.ENDC)
            return

        webxml = minidom.parse(
            "%s/target/verify/WEB-INF/web.xml" % module.get_directory())

        ujiappsFilter = [node for node in webxml.getElementsByTagName(
            "param-value") if node.childNodes[0].nodeValue == "ujiapps.uji.es"]

        if len(ujiappsFilter) != 2:
            logging.warning(
                Colors.WARNING + "[WARN] Missing ujiapps.uji.es reference on web.xml. Public module?" + Colors.ENDC)
        ...
        beansListener = [node for node in webxml.getElementsByTagName(
            "listener-class") if node.childNodes[0].nodeValue == "es.uji.commons.rest.listeners.CleanUpOracleMBeansListener"]

        if len(beansListener) != 1:
            raise Exception(
                "Must include Oracle MBean cleanup listener on web.xml")
